...
X = DefineNumber[ 0.8, Name "Parameters/X" ];
...
// Compute intersection of flag and pole
alpha = Asin(Ly/2./5);
eps = Cos(alpha)*R;
...
p2=newp; Point(p2) = {eps, -Ly/2, 0, Lc_pole};
p3=newp; Point(p3) = {X+Lx, -Ly/2, 0, Lc_flag};
p4=newp; Point(p4) = {X+Lx, 0, 0, Lc_tail};
p5=newp; Point(p5) = {X+Lx, Ly/2, 0, Lc_flag};
p6=newp; Point(p6) = {eps, Ly/2, 0, Lc_pole};
p7=newp; Point(p7) = {X, Ly/2, 0, Lc_pole};
...
loopflag = newl; Curve Loop(loopflag) = {l1, l2, l3, l4, l5, l6, l7};
flag = news; Plane Surface(flag) = {loopflag};

// create tail
p=newp; Point(p) = {X+Lx+Ltail, 0, 0, Lc_tail};
tail=newl; Line(tail) = {p4, p};
looptail = newl; Curve Loop(looptail) = {tail, -tail};

// create pole
pc=newp; Point(pc) = {-R, 0, 0, Lc_pole};
c1=newl; Circle(c1) = {p6, O, pc};
c2=newl; Circle(c2) = {pc, O, p2};

looppole = newl; Curve Loop(looppole) = {c1, c2, -l1, -l7, -l6};
