...
parser.add_argument("-end", action="store_true")  # on/off flag
...
s = gmsh.model.occ.addRectangle(r1 - eps, -cut / 2.0, 0, r2 - r1 + 2 * eps, cut)
...
for i in range(npts + 1):
    ...
    p = gmsh.model.occ.addPoint(
        r * math.cos(sens * theta), r * math.sin(sens * theta), z
    )
    # points.append(p)
...
# create cylinder
cint = gmsh.model.occ.addCylinder(0, 0, -dz / 2.0, 0, 0, dz, r1)
cext = gmsh.model.occ.addCylinder(0, 0, -dz / 2.0, 0, 0, dz, r2)
...
if args.end:
    print("add end hole")
    wire_end = gmsh.model.occ.addCylinder(
        r1 - eps,
        0,
        +nturns * pitch / 2.0 + 2 * cut - cut / 2.0,
        (r2 - r1) + 2 * eps,
        0.0,
        0,
        2 * cut,
    ...
    outDimTags, outDimTagsMap = gmsh.model.occ.fragment(
        hcut, [(3, wire_end)], removeObject=True, removeTool=True
    )
    print(f"hcut fragment: outDimTags={outDimTags}, outDimTagsMap={outDimTagsMap}")
    outDimTags, outDimTagsMap = gmsh.model.occ.fuse(
        [outDimTags[0]],
...
for id in entities:
    print(f"id: {id}")
    if id[1] not in cyl_children_id:
...
for i in volume_ids:
    boundaries = gmsh.model.getBoundary(
        [(3, i)], combined=False, oriented=False, recursive=False
    )
...
if args.mesh:
    ...
    lc = abs(r2 - r1) / 3.0
    lc_glue = lc / 4.0  # cut/2.
    print(f"lc (all)={lc}")
    print(f"lc (glue)={lc_glue}")

    gmsh.model.mesh.setSize(gmsh.model.getEntities(0), lc)
    ids = gmsh.model.getBoundary(
        [(3, volume_ids[1])], combined=False, oriented=False, recursive=True
    )
